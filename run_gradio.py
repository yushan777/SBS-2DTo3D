...
with gr.Blocks(title="SBS 2D To 3D") as demo:
    ...
    with gr.Tabs():
        ...
        |---------------|-----------------|
        | `Model` | Selects the Depth Anything V2 model for depth map generation. Different models (VITS (small), VITB (base), VITL (large)) offer trade-offs in speed and accuracy. FP16 versions are faster with slightly lower precision than FP32.<br> "If it does not exist, it will download the selected model into the directory `models/depthanything`. The default `...vitl-fp16` variant is approx 600MB. <br>Note: If you are running on Apple Silicon, it seems to prefer the fp32 variant of the models otherwise it seems to crash" |
